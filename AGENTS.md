...
- Edit sessions must leave a publish-readable `PUBLISH TRACE` in the final response: active slide ID, visible text markers, selectors/properties, registry keys, asset paths, source verification, and visual QA result. For multi-element requests, every requested element must be named and verified.
